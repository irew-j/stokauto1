...
slack = Slacker('xoxb-2173600486373-2161919017863-2TIFYx7q3R7GMw5gu4YHi1kr')
def dbgout(message):
...
def get_target_price(code):
    ...
    try:
        ...
        target_price = today_open + (lastday_high - lastday_low) * 0.5 # 고가 - 저가 * k
        return target_price
            ...
            if t_start < t_now < t_sell :  # AM 09:05 ~ PM 03:15 : 매수
                for sym in symbol_list:
                    if len(bought_list) < target_buy_count:
                        buy_etf(sym) #타이밍을 보고 매수
                        time.sleep(1)
                if t_now.minute == 30 and 0 <= t_now.second <= 5: 
                    get_stock_balance('ALL') #30분 마다 보여줌
                    time.sleep(5)
